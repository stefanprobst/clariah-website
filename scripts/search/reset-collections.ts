...
async function main() {
	...
	log.success(`Successfully reset collection "${env.NEXT_PUBLIC_TYPESENSE_COLLECTION_NAME_RESOURCES}".`);

	(await admin.collections.website.reset()).unwrap();
	log.success(`Successfully reset collection "${env.NEXT_PUBLIC_TYPESENSE_COLLECTION_NAME_WEBSITE}".`);
}
