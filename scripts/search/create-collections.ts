...
async function main() {
	...
	log.success(`Successfully created collection "${env.NEXT_PUBLIC_TYPESENSE_COLLECTION_NAME_RESOURCES}".`);

	(await admin.collections.website.create()).unwrap();
	log.success(`Successfully created collection "${env.NEXT_PUBLIC_TYPESENSE_COLLECTION_NAME_WEBSITE}".`);
}
